...
int applyCalibration(int joint, int value) {
  if (reversed[joint]) value = maxPos[joint] - (value - minPos[joint]);
  value += offsets[joint];
...
void parseMoveCommand(char* args) {
  int values[NUM_JOINTS], count = 0;
  char* token = strtok(args, " ");
  while (token != NULL && count < NUM_JOINTS) {
    values[count++] = atoi(token);
    token = strtok(NULL, " ");
  }
  if (count != NUM_JOINTS) { Serial.println("ERR BAD_ARGS"); return; }
  for (int i = 0; i < NUM_JOINTS; i++) targetPos[i] = clampJoint(i, values[i]);
...
void parseCommand(String line) {
  ...
  if (strncmp(buf, "M ", 2) == 0) { parseMoveCommand(buf + 2); return; }
  if (strncmp(buf, "OFFSET ", 7) == 0) {
    int j, val; sscanf(buf + 7, "%d %d", &j, &val);
    if (j >= 0 && j < NUM_JOINTS) { offsets[j] = val; Serial.println("OK OFFSET"); return; }
  }
  if (strncmp(buf, "REV ", 4) == 0) {
    int j, val; sscanf(buf + 4, "%d %d", &j, &val);
    if (j >= 0 && j < NUM_JOINTS) { reversed[j] = (val != 0); Serial.println("OK REV"); return; }
  }
  if (strncmp(buf, "SPEED ", 6) == 0) {
    int newInterval = atoi(buf + 6);
    if (newInterval >= 5 && newInterval <= 100) { stepIntervalMs = newInterval; Serial.println("OK SPEED"); return; }
    Serial.println("ERR BAD_SPEED"); return;
  }
